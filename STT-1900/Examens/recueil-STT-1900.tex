...
\newenvironment{H24}{\showmoduleifneeded}{}
...
\begin{A22}
\question
(A22) Considérons le montage électronique représenté ci-dessous.
...
\begin{H26}
\question
(H26) Dites si chaque énoncé est vrai ou faux.
...
\begin{H26r}
\question
(H26r) Dites si chaque énoncé est vrai ou faux.
...
\begin{parts}
    ...
    \end{solution}
        ...
        \part[1] Si $P(A \cup B) = 1$, alors $A$ est le complément de $B$.
...
\begin{H26r}
\question
(H26r) Un sac contient 30 dés rouges, 50 dés verts et 20 dés bleus.
...
\end{parts}
\end{H26r}
\moduleheader{mod2}{Module 2 - Variables aléatoires à une dimension}
\begin{H23}
...
\begin{H25}
\question
(H25) En analyse de survie, nous définissons la fonction de survie $S(x)$ comme étant la probabilité de survivre plus de $x$ unités de temps, c'est-à-dire $S(x)=P(X>x)$, où $X$ est une variable aléatoire continue qui mesure la durée de vie.  Soit la variable aléatoire $X$, dont la fonction de survie est donnée par 
\[ S(x) =
\begin{cases}
1 & \text{si } x<1\\
   \displaystyle -x^2+2x& \text{si } 1\leq x\leq 2, \\
...
\begin{H25}
\question
(H25) Soit la variable aléatoire $X$ et sa fonction de répartition
...
\begin{H26r}
\question
(H26r) Soit $X$ une variable aléatoire continue de densité $f_X$ définie ci-dessous.
...
\end{parts}
\end{H26r}
\moduleheader{mod3}{Module 3 - Loi normale et théorème central limite}
\begin{A23}
\question (A23)
...
\begin{checkboxes}
		\choice $N(92, \; 9)$  %  X1
		\CorrectChoice $N(96,\;9)$ % X_2 | X1
		\choice $N(84,\; 25)$ % X_2 
		\choice $N(100, \; 9)$ % X_1 | X2
		\choice $N(144, \; 9)$ % X_2 | X1 avec sigma^2 plutot que sigma
	\end{checkboxes}
...
\begin{H25}
\question
(H25) Soit les variables aléatoires $X\sim\mathcal{N}(70, 100)$ et $Y\sim\mathcal{N}(80, 64)$.
Calculez $P(X>Y)$.
...
\end{parts}
\end{H26r}
\moduleheader{mod4}{Module 4 - Covariance et corrélation}
...
\begin{center}
\begin{tabular}{ccc}
    \includegraphics[width=2.2in, height=2.45in]{FigureA.png} &
    \includegraphics[width=2.2in, height=2.45in]{FigureB.png} &
    \includegraphics[width=2.2in, height=2.45in]{FigureC.png} \\
    \includegraphics[width=2.2in, height=2.45in]{FigureD.png} &
    \includegraphics[width=2.2in, height=2.45in]{FigureE.png} &
    \includegraphics[width=2.2in, height=2.45in]{FigureF.png} \\
\end{tabular}
\end{center}
\end{A25}
...
\end{parts}
\end{H26r}
\moduleheader{mod5}{Module 5 - Statistiques descriptives}
\begin{A23}
\question[] (A23)
...
\begin{A24}
\question
(A24) Soit le jeu de données $x_1,\hdots,x_n$, où $n$ est une nombre impair. Nous supposons que toutes les valeurs sont différentes. La moyenne échantillonnale de ce jeu de données est $\overline{x}$ et sa médiane est $\widetilde{x}$.
...
\begin{H26r}
\question
(H26r) Soit l'histogramme ci-dessous. \\
...
\end{solution}
\end{H26r}
\moduleheader{mod6}{Module 6 - Échantillons aléatoires, lois échantillonnales et estimation ponctuelle}
...
\begin{checkboxes}
\choice $\frac{W-\mu}{\sigma}\sim\mathcal{N}(0,1)$
\CorrectChoice $3Y+10\sim \mathcal{N}(30\mu + 10; 90\sigma^2)$ 
\choice La moyenne échantillonnale $W$ est toujours égale à $\mu$
\choice Comme $n<30$, $Y$ et $W$ ne suivent pas des lois normales
\end{checkboxes}
...
\begin{parts}
    \part Si $X_1,...,X_{17}$ constituent un échantillon aléatoire simple issu d'une distribution $\mathcal{N}(5; 4)$, alors la variable $4S^2$ a une espérance de 5 et une variance de 4.
    ...
    \part Si $W$ suit une $\chi^2$ avec $5$ degrés de liberté, alors $P(W>5)=0,5$. 
    ...
    \part Si $T\sim t_{14}$, alors $P(T>14)=P(T<-14)$.
...
Soit $X_1, \dots, X_9$ un échantillon aléatoire d'une variable $X$ de loi Normale $\mathcal{N}(12, 4)$. Nous avons que $X_1, \dots, X_9$ sont indépendantes. 
...
\begin{H25}
\question
(H25) Soit deux échantillons aléatoires indépendants provenant de populations normales, tels que:
\begin{align*}
    X_1,\hdots,X_{16}&\sim\mathcal{N}(7, 4),\\
    Y_1,\hdots,Y_{100}&\sim\mathcal{N}(8, 9).
\end{align*}
...
\begin{H26}
\question
(H26) Soit $X_1,\hdots,X_{121}$, des variables aléatoires provenant d'une loi normale d'espérance $\mu=0$ et de variance $\sigma^2=16$. Quelle est la probabilité que $\displaystyle\frac{\overline{X}}{S}$ soit plus grand que $0,18$, où $S$ est l'écart-type échantillonnal?
...
\begin{H26r}
\question
(H26r) Soit $X_1,\hdots,X_5$ un échantillon de taille $n=5$ provenant d'une variable aléatoire de fonction de répartition $F_X$, d'espérance $\mu$ et de variance $\sigma^2$.
...
\begin{H26r}
\question
(H26r) Soit $X_1,\hdots,X_{121}$, des variables aléatoires provenant d'une loi normale d'espérance $\mu=0$ et de variance $\sigma^2=25$. Quelle est la probabilité que $\displaystyle\left|\frac{\overline{X}}{S}\right|$ soit plus grand que $0{,}26$, où $S$ est l'écart-type échantillonnal?

    \begin{solution}
        \begin{align*}
            P\left(\left|\frac{\overline{X}}{S}\right|>0{,}26\right)&=P\left(\left|\frac{\overline{X}}{S/\sqrt{121}}\right|>0{,}26\times 11\right)\\
            &= P\left(\left|t_{120}\right|>2,860\right)\\
            &= 2\times P\left(t_{120}>2,860\right)\\
            &=2\times 0{,}0025\\
            &=0{,}005
        \end{align*}
    \end{solution}
\end{H26r}
\moduleheader{mod7}{Module 7 - Intervalles de confiance}
...
\begin{parts}
    ...
    \part[5] Quel devrait-être le niveau de l'intervalle de confiance pour qu'il soit de longueur 0,708?
...
\begin{checkboxes}
    \choice La probabilité que $\overline{Y}$ soit dans l'intervalle de confiance est $(1-\alpha)$
%    \choice L'intervalle de confiance est centré en $\overline{Y}$
    \choice Nous savons que la proportion des $Y$ se retrouvant dans l'invervalle de confiance est $(1-\alpha)$
    \CorrectChoice Nous savons que la probabilité que $\mu$ soit dans l'intervalle de confiance est $(1-\alpha)$
    \choice Si $n$ augmente, la longueur de l'intervalle de confiance augmente
    \choice Si $\alpha$ augmente, la longueur de l'intervalle de confiance augmente
    \choice Nous savons que l'intervalle de confiance est centré en $\mu$
\end{checkboxes}
...
\begin{H25}
\question
(H25) On a noté la fréquence cardiaque de neuf hommes. 
L'intervalle de confiance à 90\% obtenu pour la fréquence cardiaque moyenne est $[94; 112]$.
On suppose que la loi normale est adéquate pour cette variable aléatoire.
...
\begin{H25}
\question
(H25) On veut calibrer une certaine machine pour qu'elle soit le plus stable possible.
On veut d’abord estimer la variation de la taille des pièces produites à l'aide d'un intervalle de confiance, en supposant que la loi normale modélise bien la variable aléatoire en question.
Un échantillon de taille 25 a donné une variance échantillonnale de 0,03642 cm$^2$.
La borne inférieure de l'intervalle de confiance pour la variance est $0,024$.
...
\begin{H26}
\question
(H26) On a noté la fréquence cardiaque de neuf hommes. 
L'intervalle de confiance à 90\% obtenu pour la fréquence cardiaque moyenne est $[94; 112]$.
On suppose que la loi normale est adéquate pour cette variable aléatoire.
...
\begin{H26}
\question
(H26) Soit $X_1,\hdots, X_{100}$, des variables aléatoires indépendantes et identiquement distribuées selon une loi normale d’espérance $\mu$ et de variance $\sigma^2 = 400$. 
...
\begin{H26r}
\question
(H26r) On a mesuré le temps de réaction (en millisecondes) de seize étudiants. 
...
\begin{H26r}
\question
(H26r) Soit $X_1,\hdots, X_{100}$, des variables aléatoires indépendantes et identiquement distribuées selon une loi normale d'espérance $\mu$ et de variance $\sigma^2 = 400$. 
...
\begin{A23}
\question
(A23) Soit $X_1,\hdots, X_{16}$, des variables aléatoires indépendantes de loi normale d'espérance $\mu$ et de variance $\sigma^2$. Voici plusieurs intervalles:
\begin{center}
...
\begin{H23}
\question
(H23)  Aurélien suppose que le temps d'attente à l'hôpital suit une loi exponentielle de moyenne 240 minutes. 
...
\begin{H23}
\question
(H23) Soit $X_1,\hdots, X_{36}$, des variables aléatoires indépendantes et identiquement distribuées selon une loi normale d'espérance~$\mu$ et de variance $\sigma^2=1296$.
...
\begin{H23}
\question
(H23) Supposons que les variables aléatoires $X_1,...,X_{20}$ suivent une loi exponentielle décalée de paramètre $\theta \geq0 $, dont la \textbf{fonction de densité} est donnée par 
...
% Fragment déplacé (régression) — commenté pour éviter l'erreur de compilation
% \begin{align*}
...
\medskip 
 ...
 \end{parts}
\end{A24}
\moduleheader{mod9}{Module 9 - ANOVA à un facteur}
\begin{A23}
...
\begin{H23}
\question
(H23) Soit la table d'ANOVA suivante obtenue à l'aide de \texttt{Rcmdr}:
\begin{verbatim}
>summary(ANOVAModel)
            Df   Sum Sq   Mean Sq   F value    Pr(>F)    
...
\vspace{6cm}
\texttt{AA}=\texttt{BB}=\texttt{CC}=\texttt{DD}=\vspace{0.3cm}
\begin{solution} 
\begin{enumerate}\itemsep0.4cm
    \item[(i)]  A1 = 4-1 = 3
    \item[(ii)] A2 = 5x4 -4 = 16
...
\end{parts}
\end{A23}
\moduleheader{mod10}{Module 10 - Régression linéaire simple}
\begin{A23}
\question (A23)
...
\part Avant d'interpréter ces résultats, le statisticien vous montre les figures suivantes:
 ...
 \hspace{-1cm}\includegraphics[scale = 0.75]{RegSimple.jpeg}

\begin{subparts}
    \subpart[1] Choisissez une figure permettant de valider le postulat de la normalité?
    ...
    \subpart[1] Choisissez une figure permettant de valider le postulat de l'homoscédasticité?
    ...
    \subpart[1] Choisissez une figure permettant de valider le postulat de la linéarité?
...
\begin{subparts}
    ...
    \subpart[1] l'estimation de la variance $\sigma^2$ représente la variabilité des observations autour de $\overline y$ et se calcule avec $\sum\limits_{i=1}^{n}(y_i-\overline y)^2/(n-2)$.
...
%%%%%%%%%%%%%%%% QUESTION %%%%%%%%%%%%%%%%
\end{A25}
\moduleheader{mod11}{Module 11 - Régression linéaire multiple}
...
\begin{subparts}
    \subpart[1] Quelle est la valeur p (p-value) associée à ce test?
...
%%%%%%%%%%%%%%%% FIN DES QUESTIONS %%%%%%%%%%%%%%%%
\end{A22}
\moduleheader{multi}{Plusieurs modules}
\begin{H23}
...
\begin{H23}
\question
(H23) Laquelle des affirmations suivantes est vraie?
\begin{checkboxes}
...
\begin{H23}
\question
(H23) Déterminez si les énoncés suivants sont vrais ou faux.
...
\begin{H24}
\question
(H24)  Quelle association de scénario et de table d'ANOVA est la bonne? Ici, A1 veut dire que le scénario A est associé à la table d'ANOVA 1, par exemple.
...
\begin{H24}
\question
(H24) Au seuil de 5\%, quelle conclusion pouvons-nous tirer de la table d'ANOVA 1? Une seule réponse est bonne.
\begin{checkboxes}
...
%%%%%%%%%%%%%%%% QUESTION %%%%%%%%%%%%%%%%
\end{H24}
\modulegroupheader{hors}{Les modules suivants ne sont plus à l'étude depuis A25}
\moduleheader{lois}{Lois conjointes}
...
\begin{H23}
\question
(H23)  Soit $X$ et $Y$ deux variables aléatoires dont la loi conjointe est 
$$f(x,y)=\left\{
			\begin{array}{ll}
...
\begin{align*}
    f_Y(y)=&\frac{\lambda^2 \alpha y^{\alpha-1}}{(1+\lambda y^\alpha)^2}\int_0^{\infty} \exp(-\lambda x) dx\\
        =& \frac{\lambda^2 \alpha y^{\alpha-1}}{(1+\lambda y^\alpha)^2} \left. \frac{-\exp(-\lambda x)}{\lambda}\right|_{x=0}^{\infty}\\
        =& \frac{\lambda \alpha y^{\alpha-1}}{(1+\lambda y^\alpha)^2}(0--1)
\end{align*}
   $$f_Y(y)=\left\{
			\begin{array}{ll}
		\displaystyle	\frac{\lambda \alpha y^{\alpha-1}}{(1+\lambda y^{\alpha})^2}  & \mbox{ si } 0\leq y \\
...
\end{parts}
\end{H25}
\moduleheader{anova2}{ANOVA deux facteurs}
...
\draw (0,0) -- (1,0) -- (1,1) -- (0,1) -- cycle; 
\end{tikzpicture}
\part[2] La figure qui correspond à la {\bf Sortie C} est la Figure \begin{tikzpicture}[baseline=4mm]
\draw (0,0) -- (1,0) -- (1,1) -- (0,1) -- cycle; 
\end{tikzpicture}
\part[2] La figure qui correspond à la {\bf Sortie D} est la Figure \begin{tikzpicture}[baseline=4mm]
\draw (0,0) -- (1,0) -- (1,1) -- (0,1) -- cycle; 
\end{tikzpicture}
\end{parts}
\begin{solution}
...
\vspace{3cm}
Estimation de la variance = \vspace{0.5cm}
\part[2] Trouvez la valeur de $k$, le nombre de logements évalués par combinaison des facteurs. Justifiez votre réponse.
...
\vspace{4cm}
\begin{solution}
    $F_0 = \frac{MSA}{MSE} = \frac{395,5/3}{160,5} = 0,8214$
\end{solution}
...
\end{parts}
\end{A24}
\moduleheader{anovablocs}{ANOVA blocs}
...
\begin{H23}
\question\label{AOVblocs2}
(H23) Quelle conclusion peut-on tirer à partir de cette table d'ANOVA, au seuil de $1\%$?
...
\begin{H23}
\question\label{AOVblocs3}
(H23) À la lumière des résultats obtenus, est-ce qu'utiliser une expérience par blocs aléatoires était une bonne idée et pourquoi?
...
\begin{A23}
\question
(A23) Laquelle des affirmations suivantes est vraie?
...
\begin{A23}
\question
(A23) Quelle conclusion peut-on tirer à partir de cette table d'ANOVA, au seuil de $1\%$?
...
\begin{A23}
\question
(A23) À la lumière des résultats obtenus, est-ce qu'utiliser une expérience par blocs aléatoires était une bonne idée et pourquoi?
...
\part[4] La sortie \texttt{Rcmdr} ci-dessus nous indique que $P(F_{AA,BB}>1.5676)=0.24846.$ Quelles sont les valeurs de AA et BB? Justifiez votre réponse.
    ...
    %\qcmrep{}
\vspace{4cm}
\texttt{AA}=\texttt{BB}=\begin{solution}    
AA = 4-1=3.\\ BB = (4-1)(5-1) = 12 
\end{solution}
