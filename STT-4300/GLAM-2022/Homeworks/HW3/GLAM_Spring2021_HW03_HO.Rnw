...
Prof. Eva Cantoni}
\rhead{GSEM\\ Spring 2021 \\ \textbf{Homework 03}}
...
mysummary(x)$'mean of x' # since the result of our function is a list.

mysummary(x)[[2]] # alternative way of retrieving the second element. 
@
}
...
loglikelihood <- function(beta, x, y){
  ...
  for (i in 1:n){
    etai <- -x[i, ] %*% beta
    loglike <- loglike + (1 - y[i]) * etai - log(1 + exp(etai))
  }
  return(-loglike) # optim minimizes - we need to put a minus
}
@
...
grid.beta0 = seq(-10,10, by=0.01)  #a grid of beta0
grid.beta1 = seq(-10,10, by=0.01)  #a grid of beta1 

profLik.beta.0=rep(0,times=length(grid.beta0))
profLik.beta.1=rep(0,times=length(grid.beta1))

for(i in 1:length(grid.beta0)){ 
  profLik.beta.0[i]=as.numeric(
    logLik(
      glm(
        cbind(returned,offered-returned)~ 
        +log(bid)-1+offset(grid.beta0[i]*rep(1,11)),
        data=goose.data, 
        family=binomial(link="logit"))))
}

for(i in 1:length(grid.beta1)){
 profLik.beta.1[i]=as.numeric(
   logLik(
     glm(
      cbind(returned,offered-returned)~
        1+offset(grid.beta1[i]*log(bid)),
      data=goose.data,
      family=binomial(link="logit"))))
}
@
