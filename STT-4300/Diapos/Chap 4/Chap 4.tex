...
\begin{center}
{\textbf{\LARGE Chapitre 4: Estimateur des $k$ plus proches voisins et estimateur de noyau}}
\end{center}
...
\noindent Ce graphique montre que la relation entre $X$ et $Y$ n'est clairement pas \sout{clairement pas }lin\'eaire. Dans ce chapitre, on consid\'erera \sout{plus}\textcolor{red}{plusieurs} approches pour \'etudier la relation entre $X$ et $Y$ dans cette situation.
...
\noindent Supposons maintenant qu'on a deux variables explicatives. Les donn\'ees sont donc $\{(Y_i,X_{i1},\sout{X}\textcolor{red}{X_{}}\sout{\{i2\}}\textcolor{red}{i2}), i=1,\cdots,n\}$ et le mod\`ele est 
$$Y_i = f(X_{i1},X_{i2})+\epsilon_i;$$
\noindent avec $E[\epsilon_i|X_{i1},X_{i2}]=0$. L'estimateur des $k$ plus proches voisins de $f$ est quasiment \sout{identitique}\textcolor{red}{identique} au cas avec une seule variable explicative. La seule diff\'erence est la fa\c{c}on de d\'efinir les distances de proximit\'e $d_i(x)$. 

\noindent Dans cette situation, $d_i(x)$ est la distance entre $x=(x_1,x_2)^\top$ et $(X_{i1},\sout{X_{i2)^\top}}\textcolor{red}{X_{i2})^\top}$. La premi\`ere id\'e\textcolor{red}{e} est de poser
\begin{eqnarray*}
...
\noindent Ces graphiques nous montr\sout{e}\textcolor{red}{ent} qu'on doit choisir le param\`etre $h$. Ceci est fait avec la m\'ethode de validation crois\'ee. Si on exclut une seule observation \`a la fois, on montre que l\sout{k}\textcolor{red}{}e crit\`ere de validation crois\'ee s'\'ecrit
