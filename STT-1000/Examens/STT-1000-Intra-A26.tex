...
Déterminez les ensembles suivants :\\
\begin{parts}
\part[3] \(A \cap B^c\)
\begin{solution}
On a
...
\question[]\label{densitetriangle} Soit \(X\) une variable aléatoire continue dont la fonction de répartition est donnée par
\[
F_X(x) = \begin{cases}
0 & \text{si } x < 0,\\
\displaystyle\frac{x^2}{8} & \text{si } 0 \le x < 2,\\
1 - \displaystyle\frac{(4-x)^2}{8} & \text{si } 2 \le x \le 4,\\
1 & \text{si } x > 4.
\end{cases}
\]
...
\part[4] Calculez la densité \(f_X(x)\) associée à cette fonction de répartition.
 
\begin{solution}
On dérive \(F_X\) sur chacun des morceaux :
\[
f_X(x) = F_X'(x) = \begin{cases}
\displaystyle\dfrac{d}{dx}\left(\displaystyle\dfrac{x^2}{8}\right) = \displaystyle\dfrac{x}{4} & \text{si } 0 \le x < 2,\\[2mm]
\displaystyle\dfrac{d}{dx}\left(1-\displaystyle\dfrac{(4-x)^2}{8}\right) = \displaystyle\dfrac{4-x}{4} & \text{si } 2 \le x \le 4,\\[2mm]
0 & \text{sinon.}
\end{cases}
\]
\end{solution}
...
\part[4] Calculez \(\mathbb{P}(1 < X < 3)\).
...
\]
\[
F_X(1) = \displaystyle\frac{1^2}{8} = 0{,}125, \qquad F_X(3) = 1-\displaystyle\frac{(4-3)^2}{8} = 1-\displaystyle\frac{1}{8} = 0{,}875.
\]
\[
\boxed{\mathbb{P}(1<X<3) = 0{,}875-0{,}125 = 0{,}75.}
\]
\end{solution}
...
\begin{solution}
\begin{align*}
\mathbb{E}(X) &= \int_0^2 x\cdot\frac{x}{4}\,dx + \int_2^4 x\cdot\frac{4-x}{4}\,dx\\
...
\question[6]
Une chaîne de production remplit des sacs de farine dont le poids \(X\) a une espérance de \(\mu = 1000\) g et une variance de \(\sigma^2 = 225\) g\(^2\). On prélève un échantillon aléatoire de \(36\) sacs.

Quelle est la probabilité que le poids moyen de l'échantillon soit inférieur à \(995\) g?
