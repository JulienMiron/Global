...
Un jeu de données est habituellement constitué de \\\alerter{plusieurs observations} de \alerter{plusieurs variables}. \\
...
Une bonne analyse est précédée d'un \alerter{résumé} des principales caractéristiques de la distribution des observations.
...
Mesure de position la plus commune: \alerter{moyenne échantillonnale} {\small (version empirique de la moyenne théorique d'une population, $\mu$).}

\begin{Boite}{Moyenne échantillonnale}
Soit un échantillon observé $x_1,\hdots,x_n$. \\
...
\item Peut être représentative du groupe... ou non 
\end{itemize}
\vspace{5mm}

\begin{tabular}{cc}
\includegraphics[width=5.1cm]{moy.png}\hspace{5 mm}& \includegraphics[width=5.1cm]{moy2.png}\\ &\\
...
\begin{itemize}
\item Médiane théorique: associée à la distribution de $X$, au modèle probabiliste de la population.
\item Médiane échantillonnale: calculée à partir d'une série d'observations $x_1,\hdots, x_n$.
\end{itemize}
\vspace{3mm}
...
\includegraphics[width=6cm]{loi2.png}

\medskip
\includegraphics[width=6cm]{loi1.png}
...
\begin{tabular}{cc}
\includegraphics[width=5.1cm]{moy.png}\hspace{5 mm}& \includegraphics[width=5.1cm]{moy3.png}\\ &\\
...
\alerter{Mode:} valeur la plus fréquente dans l'échantillon. Dénoté $m$.

\vspace{0.3 in}
Note: Il peut y avoir plus d'un mode. S'il n'y a qu'un mode, on dit que la distribution est unimodale.
...
\end{Boite}
\small{
Si $an+0,5$ n'est pas entier, on peut faire une interpolation linéaire entre les deux valeurs entières qui l'entourent. On obtient ainsi une moyenne pondérée de $x_{([an+0,5])}$ et $x_{([an+0,5]+1)}$, o\`u $[u]$ est la partie entière de $u$.
}
\end{frame}
...
\item Le $2^{e}$ quartile est le quantile d'ordre 0,50 ($50^e$ centile):\\ \alert{$Q_2=\tilde{x}=q_{0,50}$} (oui, c'est la médiane!)
...
{\small $5,~ 15,~ 17,~ 15,~  9,~  2,~  2,~ 11,~  4,~  1,~ 4.$}\hspace{4mm}\includegraphics[width=5.4cm]{dot2.png}
...
%\medskip
Deux types d'asphalte peuvent avoir des durées de vie moyennes ou médianes très similaires, mais la durée de vie d'un type d'asphalte serait beaucoup plus variable que l'autre.

\medskip
En plus des mesures de position, une autre caractéristique importante d'une
distribution est  \alerter{la dispersion des valeurs autour du centre de la distribution}.
\end{frame}
...
\item Si l'histogramme est en forme de cloche (\alerter{unimodal}  et \alerter{symétrique}), nous serons à l'aise avec le postulat de normalité de la population.
    \end{itemize}
\end{frame}
...
\item<2-> Soit $K$, le nombre de classes. On construit le tableau de fréquences contenant: \begin{itemize}
     ...
     \item[$f_j$] \alerter{$=n_j/n$}: fréquence relative de la classe $j$, pour $j=1, ..., K$.
         \end{itemize}
\item<3-> \alerter{La surface d'un rectangle doit être proportionnelle à sa fréquence relative. } Soit $\ell_j$, la largeur de la classe $j$, alors $\ell_j\cdot h_j = c\cdot f_j$, avec $c$ constant.
...
\begin{center}
\includegraphics[width=10cm]{histog.jpg}
\end{center}
\end{frame}
...
\includegraphics[width=4.8cm]{hist4.png}\hspace{1mm}ou\hspace{1mm}
\includegraphics[width=4.8cm]{hist5.png}
\end{frame}
...
\includegraphics[width=8cm,trim= 0mm 0mm 0mm 9mm, clip]{hist6.png}
...
{\small
Le \alerter{diagramme en boîte}  ({\em boxplot} ou diagramme de quartiles) permet aussi d'illustrer la distribution d'une série d'observations.\\
...
\end{align}
\item<3-> d'un trait (``moustache'') de $Q_3$ jusqu'à \alerter{la plus grande valeur de l'échantillon inférieure ou égale à $b_{sup}$}

\item<3-> d'un trait (``moustache'') de $Q_1$ jusqu'à \alerter{la plus petite valeur de l'échantillon supérieure ou égale à $b_{inf}$}
\end{itemize}
}
\end{frame}
...
\begin{center}
\includegraphics[width=8cm]{box-conc.png}
\end{center}
\end{frame}

\begin{frame}{Bibliographie}
 	Ce document est rédigé avec Beamer ainsi qu'une classe .cls fourni par Jérôme Soucy. Les diapositives sont adaptées des diapositives créées par Thierry Duchesne et Emmanuelle Reny-Nolin pour le cours STT-1900.
 	\printbibliography
 	\vfill
