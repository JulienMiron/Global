...
\begin{parts}
\part[2] L'erreur quadratique moyenne d'un estimateur sans biais $\hat{\theta}$ est égale à $V(\hat{\theta})$.
...
\begin{solution}
Faux. $\text{Biais}(\hat{\theta},\theta)=E(\hat{\theta})-\theta<0$ implique $E(\hat{\theta})<\theta$, donc $\hat{\theta}$ \textbf{sous-estime} $\theta$ en moyenne.
\end{solution}

\part[2] La moyenne échantillonnale $\overline{X}$ a toujours une variance plus petite que celle de n'importe quelle observation individuelle $X_i$ (lorsque $n>1$).
...
\begin{solution}
Vrai. $V(\overline{X})=\sigma^2/n<\sigma^2=V(X_i)$ pour $n>1$.
\end{solution}

\part[2] Un estimateur biaisé a toujours une erreur quadratique moyenne supérieure à celle de tout estimateur sans biais du même paramètre.
...
\begin{solution}
Faux. Un estimateur biaisé peut avoir une variance suffisamment faible pour compenser son biais, de sorte que son EQM soit inférieure à celle d'un estimateur sans biais ayant une grande variance.
\end{solution}

\part[2] $E(\overline{X})=\sigma^2/n$.
...
\begin{solution}
Faux. $E(\overline{X})=\mu$. C'est $V(\overline{X})$ qui vaut $\sigma^2/n$.
\end{solution}

\part[2] Si $X_1,\hdots,X_n$ sont des variables aléatoires indépendantes Bernoulli$(p)$, alors $\displaystyle\frac{1}{n}\sum_{i=1}^n X_i$ est un estimateur sans biais de $p$.
