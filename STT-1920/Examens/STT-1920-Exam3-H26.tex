...
\pageblanche
\question[12] Un biologiste veut vérifier s'il existe une différence de rigidité d'un certain type de disque, selon le pourcentage de \textit{Pouliovachium} qui le compose.
...
La table d'analyse de la variance suivante est produite par le logiciel utilisé par l'ingénieur.
...
\begin{solution}
La valeur-$p$ associée au test $F$ est très petite (dans la sortie complète: $4{,}6\times 10^{-10}$), donc $p<0{,}05$. On rejette $H_0$ et on conclut qu'au moins une moyenne diffère.
\end{solution}
