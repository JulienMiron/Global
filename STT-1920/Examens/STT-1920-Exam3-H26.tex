...
\question[10] QCM synthèse (une seule bonne réponse par item).

\begin{parts}
\part[2] \textbf{Test à 1 échantillon.} On teste $H_0:\mu=50$ contre $H_1:\mu\neq 50$ et on obtient une valeur-$p$ de $0{,}03$. Au seuil $\alpha=0{,}05$, quelle est la bonne décision?

\begin{oneparcheckboxes}
\choice On ne rejette pas $H_0$.
\correctchoice On rejette $H_0$.
\choice Le test est impossible à conclure sans la taille d'échantillon.
\end{oneparcheckboxes}
...
\vspace{0.5cm}
\part[2] \textbf{ANOVA à 1 facteur.} Quelle est l'hypothèse nulle du test $F$ global?
...
\vspace{0.5cm}
\part[2] \textbf{Régression linéaire simple.} Dans le modèle $Y=\beta_0+\beta_1X+\varepsilon$, que signifie $\beta_1>0$?

\begin{oneparcheckboxes}
\correctchoice En moyenne, $Y$ augmente quand $X$ augmente.
...
\vspace{0.5cm}
\part[2] \textbf{Khi-carré d'ajustement.} Pour un test d'ajustement avec $k=5$ catégories et aucun paramètre estimé à partir des données, combien y a-t-il de degrés de liberté?
...
\vspace{0.5cm}
\part[2] \textbf{Khi-carré d'indépendance.} Un test d'indépendance donne une valeur-$p$ de $0{,}002$ au seuil $5\%$. Quelle conclusion est correcte?
