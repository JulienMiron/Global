...
\vspace{0.5cm}
\part[3] Quelle conclusion peut-on tirer à partir de cette table d'ANOVA, au seuil de $1\%$?
...
\begin{solution}
\textbf{A -- Régression linéaire simple.} On veut modéliser l'effet d'une variable explicative quantitative (minutes de jeu par semaine) sur une variable réponse quantitative (note finale). Le modèle approprié est donc une régression linéaire simple.
\end{solution}

\vspace{0.5cm}
\part[3] Sophie aimerait comparer la performance de 8 types d'engrais. Elle utilise donc 5 plants de tomates pour chacun des types d'engrais (40 plants au total) et elle mesure la quantité de tomates produites.
...
\begin{solution}
\textbf{B -- Analyse de la variance à un facteur.} La variable explicative est qualitative (type d'engrais) avec 8 niveaux, et la variable réponse est quantitative (quantité de tomates). On cherche à comparer plusieurs moyennes de groupes: une ANOVA à un facteur est donc le bon choix.
\end{solution}
...
\begin{solution}
\textbf{C -- Test d'indépendance.} Les deux variables étudiées sont qualitatives (régime alimentaire et niveau d'énergie). L'objectif est de vérifier s'il existe une association entre ces deux variables catégorielles; on utilise donc un test d'indépendance du khi-carré.
\end{solution}

\vspace{0.5cm}
\part[3] Émile et Léon veulent savoir si le temps d'ascension de la côte de Sillery peut être expliqué par la puissance générée par un cycliste. Ils mesurent donc la puissance moyenne de 25 cyclistes pendant l'ascension ainsi que le temps d'ascension en secondes.
...
\begin{solution}
\textbf{A -- Régression linéaire simple.} On cherche à expliquer une variable quantitative (temps d'ascension) par une autre variable quantitative (puissance moyenne). Ce contexte correspond à une régression linéaire simple.
\end{solution}
...
\question[6] Soit $X_1,\hdots, X_{16}$ qui proviennent d'une loi normale d'espérance $\mu$ et de variance $\sigma^2$. \\
...
\end{solution}
\vspace{2cm}
\part[2] Au seuil $\alpha=0{,}05$, quelle est votre conclusion? 
...
\vspace{2cm}
\part[3] Calculez la statistique de test $D_0$. 
...
\vspace{2cm}
\part[2] Au seuil $\alpha=0{,}01$, quelle est votre conclusion? 
